...
sup-mon-r : {P : Prog} (p p' q : Pos P) → (p ≤ p') → ((p ∨ q) ≤ (p' ∨ q))
sup-mon-r .(Bot Act) .(Top Act) q (≤-step ↝-Act) = ≤-Top q
sup-mon-r .(Bot (Seq P Q)) .(Seqₗ (Bot P) Q) (Bot .(Seq P Q)) (≤-step (↝-Seq₀ P Q)) = ≤-step (↝-Seq₀ P Q)
sup-mon-r .(Bot (Seq P Q)) .(Seqₗ (Bot P) Q) (Top .(Seq P Q)) (≤-step (↝-Seq₀ P Q)) = ≤-refl (Top (Seq P Q))
sup-mon-r .(Bot (Seq P Q)) .(Seqₗ (Bot P) Q) (Seqₗ q .Q) (≤-step (↝-Seq₀ P Q)) = ≤-refl (Seqₗ q Q)
sup-mon-r .(Bot (Seq P Q)) .(Seqₗ (Bot P) Q) (Seqᵣ .P q) (≤-step (↝-Seq₀ P Q)) = ≤-refl (Seqᵣ P q)
sup-mon-r .(Seqₗ _ Q) .(Seqₗ _ Q) (Bot .(Seq _ Q)) (≤-step (↝-Seqₗ x Q)) = ≤-step (↝-Seqₗ x Q)
sup-mon-r .(Seqₗ _ Q) .(Seqₗ _ Q) (Top .(Seq _ Q)) (≤-step (↝-Seqₗ {P} x Q)) = ≤-refl (Top (Seq P Q))
sup-mon-r .(Seqₗ _ Q) .(Seqₗ _ Q) (Seqₗ q .Q) (≤-step (↝-Seqₗ p Q)) =
  ≤-trans {!!}
  {!!}
sup-mon-r .(Seqₗ _ Q) .(Seqₗ _ Q) (Seqᵣ P q) (≤-step (↝-Seqₗ x Q)) = ≤-refl (Seqᵣ P q)
sup-mon-r .(Seqₗ (Top P) Q) .(Seqᵣ P (Bot Q)) q (≤-step (↝-Seqₘ P Q)) = {!!}
sup-mon-r .(Seqᵣ P _) .(Seqᵣ P _) q (≤-step (↝-Seqᵣ P x)) = {!!}
sup-mon-r .(Seqᵣ P (Top Q)) .(Top (Seq P Q)) q (≤-step (↝-Seq₁ P Q)) = {!!}
sup-mon-r p .p q (≤-refl .p) = ≤-refl (p ∨ q)
sup-mon-r p p' q (≤-trans {_} {.p} {p''} l l') = ≤-trans (sup-mon-r p p'' q l) (sup-mon-r p'' p' q l')

-- sup-lower : {P : Prog} (p q r : Pos P) → (p ≤ r) → (q ≤ r) → ((p ∨ q) ≤ r)
-- sup-lower (Bot P) q r l1 l2 = l2
-- sup-lower (Top P) q r l1 l2 = l1
-- sup-lower (Seqₗ p Q) (Bot .(Seq _ Q)) r l1 l2 = l1
-- sup-lower (Seqₗ p Q) (Top .(Seq _ Q)) r l1 l2 = l2
-- sup-lower (Seqₗ p Q) (Seqₗ q .Q) r l1 l2 = transport (cong (λ p → p ≤ r) (sym (∨-Seqₗ p q Q))) {!!}
-- sup-lower (Seqₗ p Q) (Seqᵣ P q) r l1 l2 = l2
-- sup-lower (Seqᵣ P p) q r l1 l2 = {!!}
